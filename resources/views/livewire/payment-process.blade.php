<div class="container mx-auto px-6 py-12 max-w-4xl">
    <!-- Header -->
    <h1 class="text-3xl font-bold mb-12 text-gray-900">Proceso de Pago</h1>
    ...
    <div class="mb-12 bg-white shadow-sm rounded-lg p-6">
        ...
        <div class="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <p class="text-gray-900">{{ number_format($order->local_amount, 2) }} {{ $order->local_currency }}</p>
            </div>
            <div>
    ...
    <div class="mb-12">
        <div class="flex items-center justify-between bg-gray-100 rounded-xl p-6 border border-gray-200">
            ...
            <!-- Connector -->
            <div
                class="flex-1 h-px {{ in_array($order->status, ['confirmed', 'proof_uploaded', 'completed']) ? 'bg-green-500' : 'bg-gray-300' }} mx-4">
            </div>
            ...
            <div class="flex flex-col items-center text-center flex-1">
                ...
                <span
                    ...
                    {{ auth()->id() === $order->buyer_id ? 'Confirmar Envío de Pago' : 'Confirmar Recepción de Pago' }}
                </span>
            </div>

            <!-- Connector -->
            <div
                class="flex-1 h-px {{ in_array($order->status, ['proof_uploaded', 'completed']) ? 'bg-green-500' : 'bg-gray-300' }} mx-4">
            </div>
            ...
            <div class="flex flex-col items-center text-center flex-1">
                ...
                <span
                    class="text-sm {{ in_array($order->status, ['proof_uploaded', 'completed']) ? 'text-green-400' : 'text-gray-400' }} font-medium">Subir
                    Comprobante</span>
            </div>

            <!-- Connector -->
            <div class="flex-1 h-px {{ $order->status === 'completed' ? 'bg-green-500' : 'bg-gray-300' }} mx-4"></div>
        ...
        @if ($order->status === 'queued')
            <div class="bg-white rounded-xl p-8 border border-gray-200 mt-6">
                <div class="flex items-center mb-6">
                    ...
                    <h2 class="text-2xl font-bold text-gray-900">Orden en Cola</h2>
                </div>
                <p class="text-gray-600 mb-8 leading-relaxed">
                    Tu orden está en cola y será revisada por el trader. Por favor, confirma el envío o recepción del
                    pago para continuar.
                </p>
        ...
        @elseif ($order->status === 'confirmed' && auth()->id() === $order->buyer_id)
            <div class="mt-6">
        ...
        @elseif ($order->status === 'proof_uploaded')
            <div class="bg-white rounded-xl p-8 border border-gray-200 mt-6">
                ...
                <p class="text-gray-600 mb-8 leading-relaxed">
                    El comprobante ha sido subido. El trader está revisando la transacción. Revisa el estado de la orden
                    para más detalles.
                </p>
                <button wire:click="openStatusModal"
